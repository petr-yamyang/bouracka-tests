...
### §2.2 KRITICKY DŮLEŽITÉ: Zálohuj VŠECHNY workbooky před reinstallem

**Pozadí — proč to teď řešíme zvlášť přísně.** Předchozí ZIP měl chybu
BUG-K-006 — obsahoval staré workbooky v0.4.2 + v0.4.3 jako přebytek.
Při `Expand-Archive` se tyto staré soubory dostaly do install rootu a
PŘEPSALY tvůj workbook s tvými bugy. Tato chyba je teď strukturálně
opravená (post-flight gate v packageru), ale pro jistotu — vždycky
zálohuj VŠECHNY existující workbooky předtím než cokoli rozbalíš.

```powershell
# Zálohuj VŠECHNY workbook soubory v C:\TestAutomationSite\ a pod-složkách
$timestamp = Get-Date -Format "yyyy-MM-dd-HHmm"
$backupDir = "C:\TestAutomationSite-backup\$timestamp"
New-Item -ItemType Directory $backupDir -Force | Out-Null

Get-ChildItem -Recurse "C:\TestAutomationSite" -Filter "BOURACKA-TESTPLAN-*.xlsx" |
    ForEach-Object {
        $rel = $_.FullName.Substring("C:\TestAutomationSite\".Length).Replace("\","_")
        $dst = Join-Path $backupDir $rel
        Copy-Item $_.FullName $dst -Force
        Write-Host "Zalohovano: $($_.FullName) -> $dst"
    }

# Vypiš co máš v záloze
Get-ChildItem $backupDir
```

Každý nalezený workbook se zazálohuje s plnou cestou v názvu, ať víš
odkud pochází. Ten z `tests-source\` má tvé bugy (BUG-002 atd.) — ten
je nejdůležitější, ale zálohuj všechny pro jistotu.
...
```powershell
# DŮLEŽITÉ — explicitně řekni serveru kde je workbook (BUG-K-007 workaround).
# Bez tohoto env varu server defaultuje na v0.4.3 cestu (hardcoded v server.py),
# která neexistuje, a startup banner pak ukáže neplatnou cestu.
$env:BOURACKA_UI_WORKBOOK  = "C:\TestAutomationSite\BOURACKA-TESTPLAN-v0.4.4.xlsx"
...
[bouracka-ui]          other BOURACKA-TESTPLAN-*.xlsx copies are reference only.
```

Cesta k workbooku MUSÍ ukazovat na UI root (`C:\TestAutomationSite\BOURACKA-TESTPLAN-v0.4.4.xlsx`), NIKDY na `tests-source\`. Pokud ukazuje jinde, něco se neopravilo — pošli mi screenshot.

### §3.2 Framework filter test

Otevři `http://127.0.0.1:8424/#/run` → vyber framework `cypress` v dropdownu. Měla bys vidět **nenulový seznam TC** (dříve bylo 0). Stejně pro `playwright` a `selenium` jednotlivě.

### §3.3 Bug edit test

`http://127.0.0.1:8424/#/bugs` → klikni na řádek existujícího bugu (nebo si jeden založ a pak klikni). Otevře se edit form s předplněnými poli. Změň `Status` z `open` na `investigating` → klikni `Save changes`. Po refreshi vidíš novou hodnotu v seznamu.

Pokud má bug `linked_tc_ref`, uvidíš taky tlačítko **↻ Retest linked TC**.

---

## §4. Známé otevřené body (pro další kolo)

Z tvých nálezů 4 položky budou v v0.1.5 (následující ship):

1. **Bug → TC → Krok** — pole pro odkaz na konkrétní krok testovacího skriptu kde bug nastal
2. **Náhled TC kroků na /run** — kliknutí na název TC otevře read-only náhled designu (krok po kroku)
3. **Run console** — během běhu uvidíš lidský seznam typu `TestCaseName#1 - OK / TestCaseName#2 - NOK → Bug#n ve kroku#m`
4. **Auditovatelný detail Run výsledků** — Petr má pochybnost o validovatelnosti tvých výsledků, tohle bude další design pass

---

## §5. Kontakt

Cokoli zaškobrtne při reinstallu — pošli screenshot nebo trace bundle (UI → `/runs` → vyber běh → **Export bundle**) na petr.yamyang@gmail.com.

Dík za první kolo nálezů — tři tvrdé bugy ve dvou dnech, to je super signál pro stabilizaci.
